...
# combination on MTA's most recent schedule. LineString and Point features all include metadata that 
# provide: stop_name, stop_id and, a stop sequence integer.
...
def load_data():
    ...
    for boro in ['bronx', 'brooklyn', 'manhattan', 'queens', 'staten_island']:
        ...
        try:
            ...
            print 'error encountered connecting to MTA'
            sys.exit()
...
def write_geojson(route_id, direction, headsign, stop_list, line_list, target_dir):
    """
    ...
    with open('{}/{}_{}.geojson'.format(target_dir, route_id, direction), 'w') as outfile:
        geojson.dump(geo_dict, outfile)


def flush_old_files(target):
    """
    ...
    for geo in glob.glob('{}/*.geojson'.format(target)):
        os.remove(geo)
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        ...
        print 'usage: python rebuild_geojson.py <target_dir>'
        sys.exit()

    if os.path.isdir(target_dir):
        # data load exits script on fail
        data = load_data()
        flush_old_files(target_dir)
    else:
    ...
    for route_id in data['trips']['route_id'].unique():
        for direction in [0, 1]:
            ...
            write_geojson(route_id, direction, headsign, stop_list, line_list, target_dir)
            i+=1
